...
# MAGIC %md
# MAGIC ### Step 5. Read the data we wrote to DataLake back into a DataFrame to prove the write worked
...
if (spark._jsparkSession.catalog().tableExists("f1_delta.qualifying")):
  deltaTable = DeltaTable.forPath(spark, "/mnt/adobeadls/deltalake/qualifying")
  deltaTable.alias("tgt").merge(
    qualifying_final_df.alias("src"),
    "tgt.race_id = src.race_id AND tgt.qualify_id = src.qualify_id AND tgt.driver_id = src.driver_id") \
  .whenMatchedUpdateAll() \
  .whenNotMatchedInsertAll() \
  .execute()
else:
  qualifying_final_df.write.mode("overwrite").partitionBy("race_id").format("delta").saveAsTable("f1_delta.qualifying")
...
# MAGIC COUNT(*) AS CNT,
# MAGIC file_date  
# MAGIC FROM f1_delta.qualifying
# MAGIC GROUP BY 2;
...
# MAGIC %sql
# MAGIC DROP TABLE f1_delta.qualifying;
